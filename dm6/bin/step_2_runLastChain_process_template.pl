...
foreach my $query (@query_list)
{
	...
	foreach my $chrom (@target_list) 
	{
		...
		while($command_count < $number_of_commands)
		{
              			...
              			foreach my $host (@hosts)
				{
						...
						elsif ($pid == 0)
						{
						  # print "Printed by child process\n";
                                       		   ...
                                       		   submitThread($thread_command,$count,$host);
						   exit(0);
						}
						else 
						{
			...
			{
				waitpid($_, 0);
        			#print "-step 2- LAV2PSL done with $_\n";
			}
...
foreach my $query (@query_list)
{
        foreach my $chrom (@target_list)
        {
			...
			$count++;
			my $cmdChain="perl $target_dir\/chainJobs_${chrom}_${query}.csh > $target_dir_parent/log/chainJobs_${query}_${chrom}.log 2>&1";
...
sub lavCommand 
{
	...
	my $cmd = "$target_dir\/runLastz_${target_name}_${query_name} $target_name $query_name $query_chunk $target_chunk $LAV_FILE > $target_dir_parent/log/lavAndPsl_${LAV_FILE}.log 2>&1";
	return $cmd;
}	
sub submitThread
{
